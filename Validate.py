...
@staticmethod
def validate(model, batch_id, mode = 'training', samples_per_category = 1024, use_mle=True, use_rl = False, tensorboard = None):
    torch.seed(1111)
    np.seed(1111)
    ...
    validation_datasets = [GeneratorDataset(categories = [categories[category_index]],
                                          num_iterations=1, batch_size = samples_per_category)
                                          for category_index in range(len(categories))]
    validation_loaders = [data.DataLoader(
        validation_datasets[category_index], batch_size=samples_per_category, shuffle=True,
    ...
    if use_mle and not use_rl:
        for category_index in len(categories):
            validation_batches = [validation_loaders.[0] for validation_loader in validation_loaders]
            batch_qs, batch_as = [map(lambda x: x.to(device), validation_batch) for validation_batch in
                                  validation_batches]
            loss, n_correct, n_char = Trainer.mle_batch_loss(batch_qs[category_index], batch_as[category_index],
                                                             model.action_transformer)
            losses_by_category.append(loss)
            n_correct_by_category.append(n_correct)
            n_char_by_category.append(n_char)
        average_loss = np.mean(losses_by_category)
        average_n_correct = np.mean(n_correct_by_category)
        average_n_char = np.mean(n_char_by_category)

        if tensorboard is not None:
            tb_mle_batch(tensorboard, average_loss, average_n_char, average_n_correct, 0)
        ...
        reward = np.meanb
        if tensorboard is not None:
            tb_policy_batch(tensorboard, batch_rewards, value_losses, 0, 0, num_datapoints)
