...
class TeacherTrainer:
	...
	def __init__(self, op='mle', device='cpu', teacher_network=True, teacher_model=None, student_model=None, student_optimizer=None, teacher_lr=0.1, validation_samples=1, tb=None):
		...
		if teacher_model:
			...
			self.teacher_params = torch.Variable(torch.FloatTensor([1/num_categories]*num_categories), requires_grad=True)
	...
	def train_teacher(self, data_loader=None, K=10, task_batch_size=10, num_categories=1, num_iterations=100000):
		...
		for num_idx in tqdm(range(num_iterations), mininterval=2, leave=False):
			if self.teacher_model:
				...
				category_probs = self.teacher_params.contiguous().view(-1)

			tasks = Categorical(F.softmax(category_probs + torch.rand_like(category_probs)*0.05, dim=-1)).sample((K,))
			...
			valid_grads, valid_losses, avg_acc, avg_loss = self.create_grad_labels(tasks, valid_grads, category_acc, 
	...
	def create_grad_labels(self, tasks, valid_grads, category_acc, accs, losses, task_counts, iter_acc, iter_loss):
